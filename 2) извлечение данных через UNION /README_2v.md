...
1. Открыла страницу клиентов и сначала ввела обычный поисковый запрос `corp.local`. Так было видно, что поле действительно фильтрует клиентов по email и возвращает подходящие записи.
...
3. После этого ввела условие `' OR 1=1 --`. Конструкция `OR 1=1` делает условие всегда истинным, а `--` комментирует остаток исходного SQL-запроса. В результате были получены все данные клиентов, что подтвердило возможность менять условие `WHERE`.
