...
Также можно вывести данные другой таблицы, если знать названия таблиц, сколько колонок, это важно потому что для этой инъекции важно чтобы были совместимые типы данных и одинаковое колличество колонок. далее в типе 3) типе я нахожу названия таблиц, их колонок через пэйлоад в SQL reports. Поэтому можно вывести данные из других таблиц через эту таблицу. Код: 

```sql
' AND 1=0 UNION SELECT   0,   username,   role,   password,   id FROM training.app_users --
```
